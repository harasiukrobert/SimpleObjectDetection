...
class MyDataset(Dataset):
    def __init__(self, path, transforms=False, phase='train'):
        ...
        self.images, self.masks, self.annotations_list, self.bboxes_list = self.load_images(path)
        ...
        for file in os.listdir(path):
            if file.endswith(".png"):
                ...
                if self.phase == 'train' or self.phase == 'val':
                    mask = cv2.imread(os.path.join(f'data/{self.phase}_annotations_masks', file), cv2.IMREAD_GRAYSCALE)
                    masks_list.append(mask)

        return img_list, masks_list, annotations_list, bboxes_list
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train' or self.phase == 'val':
            ...
            mask = self.masks[idx]
            mask = (mask > 16).astype(np.float32)

            if self.transforms:
                transformed  = self.transform_true(image=img_rgb, mask=mask, bboxes=boxes)
                ...
                transformed = self.transform_false(image=img_rgb, mask=mask, bboxes=boxes)
                img_transformed = transformed['image']
                mask_transformed = transformed['mask']
                boxes_transformed = transformed['bboxes']
                if len(boxes_transformed) < 6:
